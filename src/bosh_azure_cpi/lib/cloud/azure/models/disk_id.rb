  ...
  class DiskId < ResObjectId
    ...
    def validate
      if @plain_id.nil?
        ...
        cloud_error("Invalid storage_account_name in disk id (version 2) '#{self}'") if disk_name.start_with?(DATA_DISK_PREFIX) && (storage_account_name.nil? || storage_account_name.empty?)
      end
    end
