...
module Bosh::AzureCloud
  class InstanceTypeMapper
    ...
    def map(desired_instance_size, available_vm_sizes)
      CPILogger.instance.logger.debug("The available VM sizes in the specified region are '#{_vm_sizes_to_string(available_vm_sizes)}'")
      ...
      CPILogger.instance.logger.debug("The possible VM sizes which meet desired instance size are '#{_vm_sizes_to_string(possible_vm_sizes)}'")

      closest_matched_vm_sizes = _find_closest_matched_vm_sizes(possible_vm_sizes)
      CPILogger.instance.logger.debug("The closest matched VM sizes are '#{closest_matched_vm_sizes}'")
    ...
    def _find_closest_matched_vm_sizes(possible_vm_sizes)
      ...
      CPILogger.instance.logger.debug("The recommended VM sizes are '#{_vm_sizes_to_string(vm_sizes)}'")
      ...
      cloud_error('Unable to find the closest matched VM sizes') if vm_sizes.empty?
      CPILogger.instance.logger.debug("The sorted VM sizes are '#{_vm_sizes_to_string(vm_sizes)}'")
