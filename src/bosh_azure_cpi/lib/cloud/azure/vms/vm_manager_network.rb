  ...
  class VMManager
    ...
    def _create_network_interfaces(resource_group_name, vm_name, location, vm_props, network_configurator, primary_nic_tags = AZURE_TAGS)
      ...
      nic_groups.each_with_index do |group_networks, nic_index|
        ...
        if nic_index.zero?
          ...
          has_ip_based_pools = Array(load_balancers).any? do |load_balancer|
            [:backend_address_pools, :backend_address_pools_v6].any? do |pool_key|
              pools = load_balancer[pool_key]
              next false if pools.nil?

              pools = [pools] unless pools.is_a?(Array)
              pools.any? do |pool|
                pool[:backend_address_pools_type] == LOAD_BALANCER_BACKEND_POOL_TYPE_IP
              end
            end
          end

          nic_params[:tags][LOAD_BALANCER_USED_BY_TAG] = 'true' if has_ip_based_pools
    ...
    def _remove_vm_from_load_balancer_backend_pool(virtual_machine_result)
      ...
      vm_used_by_load_balancer = virtual_machine_result[:network_interfaces].select { |nic|
        nic[:tags]&.any? {
          |key, value| key == LOAD_BALANCER_USED_BY_TAG && value.to_s.casecmp?('true')
        }
      }
