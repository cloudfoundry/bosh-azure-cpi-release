...
module Bosh::AzureCloud
  class StemcellManager
    ...
    def delete_stemcell(name)
      ...
      if @meta_store.meta_enabled
        ...
        stemcell_metas.each do |stemcell_meta|
          CPILogger.instance.logger.info("Delete stemcell #{name} in the storage #{stemcell_meta.storage_account_name}")
    ...
    def create_stemcell(image_path, stemcell_properties)
      CPILogger.instance.logger.info("create_stemcell(#{image_path}, #{stemcell_properties})")

      stemcell_name = nil
      Dir.mktmpdir('sc-') do |tmp_dir|
        CPILogger.instance.logger.info("Unpacking image: #{image_path}")
        command_runner = CommandRunner.new
        command_runner.run_command("tar -zxf #{image_path} -C #{tmp_dir}")
        CPILogger.instance.logger.info('Start to upload VHD')
        stemcell_name = "#{STEMCELL_PREFIX}-#{SecureRandom.uuid}"
        CPILogger.instance.logger.info("Upload the stemcell #{stemcell_name} to the storage account #{@default_storage_account_name}")
    ...
    def get_stemcell_uri(storage_account_name, name)
      CPILogger.instance.logger.info("get_stemcell_uri(#{storage_account_name}, #{name})")
    ...
    def get_stemcell_info(storage_account_name, name)
      CPILogger.instance.logger.info("get_stemcell_info(#{storage_account_name}, #{name})")
    ...
    def _handle_stemcell_in_different_storage_account(storage_account_name, name)
      ...
      if !stemcell_meta.nil?
        ...
        begin
          ...
          CPILogger.instance.logger.info("Copying stemcell #{name} to #{storage_account_name}")
    ...
    def _wait_stemcell_copy(storage_account_name, name, timeout = DEFAULT_COPY_STEMCELL_TIMEOUT)
      CPILogger.instance.logger.info('Another process is copying the same stemcell')
      loop do
        stemcell_meta = @meta_store.find_first_stemcell_meta(name, storage_account_name)
        ...
        if (current_time - start_time) > timeout
          CPILogger.instance.logger.info("The timestamp of the record is #{start_time}, current time is #{current_time}")
