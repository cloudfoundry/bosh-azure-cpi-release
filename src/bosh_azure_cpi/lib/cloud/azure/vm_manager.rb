    ...
    def get_or_create_availability_set(resource_group_name, availability_set_params)
      ...
      flock("#{CPI_LOCK_PREFIX_AVAILABILITY_SET}-#{availability_set_name}", File::LOCK_EX) do
        ...
        if availability_set.nil?
          @logger.info("The availability set `#{availability_set_name}' doesn't exist. Will create a new one.")
        ...
        # In some regions, the location of availability set is case-sensitive, e.g. CanadaCentral instead of canadacentral.
          ...
          cloud_error("The availability set `#{availability_set_name}' already exists, but in a different location `#{availability_set[:location].downcase}' instead of `#{availability_set_params[:location].downcase}'. Please delete the availability set or choose another location.")
        elsif !@use_managed_disks && availability_set[:managed]
          cloud_error("The availability set `#{availability_set_name}' already exists. It's not allowed to update it from managed to unmanaged.")
        elsif @use_managed_disks && !availability_set[:managed]
          @logger.info("The availability set `#{availability_set_name}' exists, but it needs to be updated from unmanaged to managed.")
          ...
          @logger.info("The availability set `#{availability_set_name}' exists. No need to update.")
        end
      end
