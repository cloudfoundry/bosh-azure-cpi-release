...
module Bosh::AzureCloud
  ...
  class AzureClient
    ...
    def create_network_interface(resource_group_name, nic_params)
      ...
      ip_configs_json = _build_ip_configurations(nic_params)
    ...
    def create_gallery_image_definition(gallery_name, image_definition, params)
      ...
      image_definition_params['properties']['architecture'] = params['architecture'] if params.key?('architecture')
      image_definition_params['properties']['features'] = params['features'] if params.key?('features')
      ...
      http_put(url, image_definition_params, { 'api-version' => '2025-03-03' })
    end
    ...
    # @param [String] gallery_name - Name of gallery.
    # @param [String] image_definition - Name of gallery image.
    # @return [Hash,nil] The gallery image definition or nil if not found
    #
    # @See https://learn.microsoft.com/en-us/rest/api/compute/gallery-images/get
    #
    def get_gallery_image_definition(gallery_name, image_definition)
      url = rest_api_url(REST_API_PROVIDER_COMPUTE, "#{REST_API_GALLERIES}/#{gallery_name}/images/#{image_definition}", resource_group_name: @azure_config.resource_group_name)
      result = get_resource_by_id(url)
      result
    end
    ...
    def _build_ip_configurations(nic_params)
      ...
      ip_configurations.each_with_index.map do |ipconfig, idx|
        ...
        # Public IP - only on primary IPv4 config
        if is_primary && nic_params[:public_ip]
          config_properties['publicIPAddress'] = { 'id' => nic_params[:public_ip][:id] }
        end
        ...
        if nic_params[:load_balancers]
          ...
          if ip_version == 'IPv4'
            inbound_nat_rules = nic_params[:load_balancers]
                                .flat_map { |lb| lb[:frontend_ip_configurations][0][:inbound_nat_rules] }
                                .compact
        ...
        if ip_version == 'IPv4' && nic_params[:application_gateways]
          agw_backend_pools = nic_params[:application_gateways].map { |agw| { 'id' => agw[:backend_address_pools][0][:id] } }
          config_properties['applicationGatewayBackendAddressPools'] = agw_backend_pools
        end
