  ...
  class StorageAccountManager
    ...
    def generate_storage_account_name
      ...
      until available
        ...
        CPILogger.instance.logger.debug('generate_storage_account_name - generating a new storage account name')
        result = @azure_client.check_storage_account_name_availability(storage_account_name)
        available = result[:available]
        CPILogger.instance.logger.debug('generate_storage_account_name - The generated storage account name is not available') unless available
      end
      CPILogger.instance.logger.debug("generate_storage_account_name - The storage account name '#{storage_account_name}' is available")
    ...
    def get_or_create_storage_account(name, tags, sku, kind, location, containers, is_default_storage_account)
      CPILogger.instance.logger.debug("get_or_create_storage_account(#{name}, #{tags}, #{sku}, #{kind}, #{location}, #{containers}, #{is_default_storage_account})")
      storage_account = nil
      flock("#{CPI_LOCK_PREFIX_STORAGE_ACCOUNT}-#{name}", File::LOCK_EX) do
        ...
        if storage_account.nil?
          CPILogger.instance.logger.debug("Cannot find any storage account with name '#{name}', creating a new one...")
          result = @azure_client.check_storage_account_name_availability(name)
          CPILogger.instance.logger.debug("get_or_create_storage_account - The result of check_storage_account_name_availability is #{result}")
          ...
          CPILogger.instance.logger.debug("Creating storage account '#{name}' with the tags '#{tags}' in the location '#{location}'")
    ...
    def get_or_create_storage_account_by_tags(tags, sku, kind, location, containers, is_default_storage_account)
      CPILogger.instance.logger.debug("get_or_create_storage_account_by_tags(#{tags}, #{sku}, #{kind}, #{location}, #{containers}, #{is_default_storage_account})")
      storage_account = nil
      ...
      flock(lock_file, File::LOCK_EX) do
        ...
        if storage_account.nil?
          CPILogger.instance.logger.debug("Cannot find any storage account in the location '#{location}' with tags '#{tags}', creating a new one...")
    ...
    def find_storage_account_by_name(name)
      CPILogger.instance.logger.debug("find_storage_account_by_name(#{name})")
      storage_account = @azure_client.get_storage_account_by_name(name)
      CPILogger.instance.logger.debug("Found storage account: '#{storage_account[:name]}'") unless storage_account.nil?
      storage_account
    end
    ...
    def find_storage_account_by_tags(tags, location)
      CPILogger.instance.logger.debug("find_storage_account_by_tags(#{tags}, #{location})")
    ...
    def default_storage_account
      ...
      unless storage_account_name.nil?
        CPILogger.instance.logger.debug("The default storage account is '#{storage_account_name}'")
      ...
      CPILogger.instance.logger.debug('The default storage account is not specified in global settings.')
      storage_accounts = @azure_client.list_storage_accounts
      location = @azure_client.get_resource_group(@azure_config.resource_group_name)[:location]
      CPILogger.instance.logger.debug("Will look for an existing storage account with the tags '#{STEMCELL_STORAGE_ACCOUNT_TAGS}' in the location '#{location}'")
      storage_account = storage_accounts.find do |s|
        s[:location] == location && is_stemcell_storage_account?(s[:tags])
      end
      unless storage_account.nil?
        CPILogger.instance.logger.debug("The default storage account is '#{storage_account[:name]}'")
        @default_storage_account = storage_account
        return @default_storage_account
      end

      CPILogger.instance.logger.debug("Can't find a storage account with the tags '#{STEMCELL_STORAGE_ACCOUNT_TAGS}'")
      CPILogger.instance.logger.debug("Will look for the old storage account (with the table '#{STEMCELL_TABLE}') which stores all uploaded stemcells")
      storage_account = storage_accounts.find do |s|
      ...
      unless storage_account.nil?
        ...
        if storage_account[:location] == location
          CPILogger.instance.logger.debug("Use an exisiting storage account '#{storage_account_name}' as the default storage account")
          CPILogger.instance.logger.debug("Set the tags '#{STEMCELL_STORAGE_ACCOUNT_TAGS}' for the storage account '#{storage_account_name}'")
    ...
    def get_or_create_diagnostics_storage_account(location)
      CPILogger.instance.logger.debug("get_or_create_diagnostics_storage_account(#{location})")
    ...
    def _has_stemcell_table?(name)
      ...
      begin
        ...
        CPILogger.instance.logger.info("_has_stemcell_table?: Calling get_table(#{STEMCELL_TABLE}, #{options})")
