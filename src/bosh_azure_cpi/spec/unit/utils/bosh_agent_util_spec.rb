...
describe Bosh::AzureCloud::BoshAgentUtil do
  ...
  subject(:agent_util) { Bosh::AzureCloud::BoshAgentUtil }
  ...
  before do
    allow(Bosh::AzureCloud::Config).to receive(:instance).and_return(config)
    allow(config).to receive(:agent).and_return({'mbus' => 'http://u:p@somewhere'})
    expect(config).to receive(:use_registry?).and_return(use_registry)
    agent_util.prepare_user_data(agent_id, network_spec, environment)
  end
  ...
  context 'when using registry' do
    ...
    it 'user data is for registry usage' do
      user_data = agent_util.get_user_data_obj(
                    registry_endpoint,
                    instance_id,
                    dns,
                    vm_params,
