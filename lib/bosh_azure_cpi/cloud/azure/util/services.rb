...
require_relative '../reserved_ip_manager'
...
module Bosh::AzureCloud::Util
  module Services
    ...
    def instance_manager
      @instance_manager ||= Bosh::AzureCloud::VMManager.new(azure_vm_client, image_service, vnet_manager, storage_manager)
    end

    def affinity_group_manager
      @ag_manager ||= Bosh::AzureCloud::AffinityGroupManager.new(base_service)
    end

    def vnet_manager
      @vnet_manager ||= Bosh::AzureCloud::VirtualNetworkManager.new(vnet_service, affinity_group_manager)
    end

    def stemcell_manager
      @stemcell_creator ||= Bosh::AzureCloud::StemcellManager.new(blob_manager, image_service)
    end

    def storage_manager
      storage_acct_name = options['azure']['storage_account_name'] || raise("Missing 'storage_account_name' from manifest")
      @storage_manager ||= Bosh::AzureCloud::StorageAccountManager.new(storage_service, storage_acct_name)
    end

    def blob_manager
      @blob_manager ||= Bosh::AzureCloud::BlobManager.new(blob_service)
    end

    def reserved_ip_manager
